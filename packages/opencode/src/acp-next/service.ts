...
function defaultModelFromConfig(
  ...
  // a default. Configured model, synora-foundry provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const foundryProvider = providers[ProviderID.make("synora-foundry")]
  const foundryModel = foundryProvider ? Provider.sort(Object.values(foundryProvider.models))[0] : undefined
  if (foundryProvider && foundryModel) return { providerID: foundryProvider.id, modelID: foundryModel.id }
