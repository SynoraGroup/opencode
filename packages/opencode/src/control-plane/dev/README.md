...
In a separate terminal, run a separate Synora workspace server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/opencode/script/run-workspace-server
```

With the plugin installed, you can now run the local harness and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
